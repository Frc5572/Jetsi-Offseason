...
import static frc.robot.util.math.Approx.approxEquals;
...
/**
 * Modified from 6328's 2025 public repo.
 */
@Builder
...
public class SwerveSetpointGenerator {
     ...
     *         (approximate) root.
     */
    private double findRoot(Function2d func, double x_0, double y_0, double f_0, double x_1,
        double y_1, double f_1, int iterations_left) {
        if (iterations_left < 0 || approxEquals(f_0, f_1)) {
            return 1.0;
        }
    ...
    protected double findSteeringMaxS(double x_0, double y_0, double f_0, double x_1, double y_1,
        double f_1, double max_deviation, int max_iterations) {
    ...
    protected double findDriveMaxS(double x_0, double y_0, double f_0, double x_1, double y_1,
        double f_1, double max_vel_step, int max_iterations) {
        double diff = f_1 - f_0;
     ...
     */
    public SwerveSetpoint generateSetpoint(final SwerveSetpoint prevSetpoint,
        ...
        if (Constants.Swerve.TUNABLE.getMaxSpeed().in(MetersPerSecond) > 0.0) {
            SwerveDriveKinematics.desaturateWheelSpeeds(desiredModuleState,
                Constants.Swerve.TUNABLE.getMaxSpeed().in(MetersPerSecond));
        ...
        boolean desiredStateIsZero = approxEquals(desiredState.vxMetersPerSecond, 0.0)
            && approxEquals(desiredState.vyMetersPerSecond, 0.0)
            && approxEquals(desiredState.omegaRadiansPerSecond, 0.0);
        if (desiredStateIsZero) {
            need_to_steer = false;
        ...
        boolean prevSetpointIsZero =
            approxEquals(prevSetpoint.chassisSpeeds().vxMetersPerSecond, 0.0)
                && approxEquals(prevSetpoint.chassisSpeeds().vyMetersPerSecond, 0.0)
                && approxEquals(prevSetpoint.chassisSpeeds().omegaRadiansPerSecond, 0.0);
        if (all_modules_should_flip && !prevSetpointIsZero && !desiredStateIsZero) {
        ...
        final double max_theta_step =
            dt * Constants.Swerve.TUNABLE.getMaxSterringVelocity().in(RadiansPerSecond);
        for (int i = 0; i < modules.length; ++i) {
            if (!need_to_steer) {
            ...
            if (approxEquals(prevSetpoint.moduleStates()[i].speedMetersPerSecond, 0.0)) {
                ...
                if (approxEquals(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
                ...
                if (numStepsNeeded <= 1.0) {
                    ...
                    overrideSteering.set(i,
                        Optional.of(
                            prevSetpoint.moduleStates()[i].angle.rotateBy(Rotation2d.fromRadians(
            ...
            double s = findSteeringMaxS(prev_vx[i], prev_vy[i], prev_heading[i].getRadians(),
                desired_vx[i], desired_vy[i], desired_heading[i].getRadians(), max_theta_step,
                kMaxIterations);
        ...
        final double max_vel_step =
            dt * Constants.Swerve.TUNABLE.getMaxAcceleration().in(MetersPerSecondPerSecond);
        for (int i = 0; i < modules.length; ++i) {
            if (min_s == 0.0) {
                // No need to carry on.
                break;
            ...
            double s = min_s * findDriveMaxS(prev_vx[i], prev_vy[i],
                Math.hypot(prev_vx[i], prev_vy[i]), vx_min_s, vy_min_s,
                Math.hypot(vx_min_s, vy_min_s), max_vel_step, kMaxIterations);
            min_s = Math.min(min_s, s);
        }

        ChassisSpeeds retSpeeds =
            new ChassisSpeeds(prevSetpoint.chassisSpeeds().vxMetersPerSecond + min_s * dx,
